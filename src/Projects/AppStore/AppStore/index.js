...
export class AppStore extends Component {
  ...
  render() {
    const { tabState, searchInput } = this.state
    const filteredApps = appsList.filter(eachItem => eachItem.category.includes(tabState))
    ...
    return (
      <div className='bg-gradient-to-r from-yellow-100 to-green-200 h-screen bg-cover overflow-auto	 p-20 px-6 md:px-16 lg:px-44 xl:px-60 py-10'>
        <div className='flex flex-col items-center h-96'>
          ...
          <button className='flex flex-row justify-center items-center border rounded border-2 border-gray-500 mb-2'>
            <input onChange={this.updatesQuery} type="text" className='py-2 pl-2' placeholder='Seach Apps' />
            <img className='w-8 ml-1 mr-1 rounded-xl' src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png" />
          </button>

          {/* Tab Items */}
          <div className='flex flex-row justify-center items-center'>
            {tabsList.map(eachItem => (
              <TabItem isTabActive={tabState === eachItem.tabId} updateTab={this.updateTab} tabsList={eachItem} key={eachItem.tabId} />
            ))}
          </div>
